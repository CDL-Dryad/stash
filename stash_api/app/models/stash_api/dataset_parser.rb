  ...
  class DatasetParser
    ...
    def add_author(json_author: author)
      ...
      if json_author[:affiliation].present?
        ror_org = find_first_by_ror_name(json_author[:affiliation]).first
        a.affiliation = StashDatacite::Affiliation.first_or_create(long_name: json_author[:affiliation], ror_id: ror_org.is_a?(Hash) ? ror_org['id'] : nil)
      end
